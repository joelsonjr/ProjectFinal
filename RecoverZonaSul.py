...
def recoverZonaSulFood(site):
    cursor.execute("delete from Acougue where id_empresa = 1;")
    ...
    for product in products:
        try:
            ...
            cursor.execute("""
                           ...
                           VALUES (1,?,?,?,?,?)""", (title, price[0], weight, site[2], site[1]))
        except AttributeError as e:
            continue

#Recuperando itens de açougue do supermercado Zona Sul
def recoverFoodZonaSul():
    foodsSites = {("https://www.zonasul.com.br/SubSecao/Bovinas--124", 1, "Carnes"),
                  ("https://www.zonasul.com.br/SubSecao/Aves--125", 1, "Aves"),
                  ("https://www.zonasul.com.br/SubSecao/Salsichas_e_Linguicas--128", 1, "Linguicas"),
                  ("https://www.zonasul.com.br/SubSecao/Suinas--123", 1, "Suínas")}
    for foodsSite in foodsSites:
        recoverZonaSulFood(foodsSite)
        page = requests.get(foodsSite[0])
        soup = BeautifulSoup(page.content, 'html.parser')
        try:
...
def recoverSpecialFoodZonaSul():
    foodsSites = {("https://www.zonasul.com.br/SubSecao/Exoticas--126", 1, "Carnes")}#,
                  #("https://www.zonasul.com.br/SubSecao/Racas_Britanicas--311, 1, "Carnes")}
    for foodsSite in foodsSites:
        recoverZonaSulFood(foodsSite)
        page = requests.get(foodsSite[0])
        soup = BeautifulSoup(page.content, 'html.parser')
...
def recoverDrinksZonaSul():
    ...
    for drinksSite in drinksSites:
        ...
        page = requests.get(drinksSite[0])
        soup = BeautifulSoup(page.content, 'html.parser')
        try:
            pages = soup.find('div', class_="paginas").find('span', id="ctl00_cphMasterPage1_dpgPromocaoTopo").find_all('a', class_='num')
            for page in pages:
...
def recoverSpecialDrinksZonaSul():
    drinksSites = {("https://www.zonasul.com.br/SubSecao/Cachacas_e_Vodkas--29", 1, "Cachaças"),
